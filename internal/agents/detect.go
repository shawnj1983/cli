...
const (
	...
	agentCursorCloud AgentName = "cursor-cloud"
...
func detectWith(lookup func(string) (string, bool)) AgentName {
	...
	// Cursor Cloud Agent
	// No first-party docs
	// CURSOR_AGENT_SOCKET is set in Cursor Cloud Agent VMs. Cursor CLI also sets
	// CURSOR_AGENT, so the socket (and bc- conversation IDs) are the signals
	// that distinguish a cloud agent from local CLI.
	if isSet("CURSOR_AGENT_SOCKET") || strings.HasPrefix(valueOf("CURSOR_CONVERSATION_ID"), "bc-") {
		return agentCursorCloud
	}

	// Cursor CLI
	// No first-party docs
	// CURSOR_AGENT is a stronger "agent is driving gh" signal than
	// CURSOR_TRACE_ID, which is also present in ordinary Cursor IDE terminals.
...
// SkillHostID maps a detected invoking agent to a `gh skill --agent` host ID.
// An empty string means there is no corresponding skill host.
func SkillHostID(name AgentName) string {
	switch name {
	case agentAmp:
		return "amp"
	case agentClaudeCode, agentCowork:
		return "claude-code"
	case agentCodex:
		return "codex"
	...
	case agentAntigravity:
		return "antigravity"
	case agentAugmentCLI:
		return "augment"
	case agentReplit:
		return "replit"
	case agentGoose:
		return "goose"
	case agentCursor, agentCursorCLI, agentCursorCloud:
		return "cursor"
	case agentKiro:
		return "kiro-cli"
	case agentPi:
